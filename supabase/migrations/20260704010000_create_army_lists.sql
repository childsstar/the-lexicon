...
begin
  if not exists (
    ...
    where tgname = 'army_lists_set_updated_at'
      and tgrelid = 'public.army_lists'::regclass
  ) then
...
begin
  if not exists (
    ...
    where schemaname = 'public' and tablename = 'army_lists'
      and policyname = 'Users can read their own army lists'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'army_lists'
      and policyname = 'Users can insert their own army lists'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'army_lists'
      and policyname = 'Users can update their own army lists'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname = 'public' and tablename = 'army_lists'
      and policyname = 'Users can delete their own army lists'
  ) then
